  ...
  useEffect(() => {
    ...
    visible.forEach((ev) => {
      ...
      el.addEventListener('click', () => {
        // keep the fly/seek behavior if you still want click to zoom/video-seek
    ...
    // ✅ proper cleanup: remove popup and all markers
    return () => {
      popup.remove();
      markersRef.current.forEach((marker) => marker.remove());
      markersRef.current = [];
        ...
        {showFeed && (
          ...
          >
            ...
            <div
              ...
              onScroll={() => setUnreadCount(0)} // clear badge when user scrolls
            >
              {events.length === 0 ? (
                // ✅ Empty state message when there are no detections
                // ✅ Empty state message when there are no detections
                <div
                  style={{
                    textAlign: 'center',
                ...
                events
                  .slice()
                  .filter((ev) => eventFilter === 'all' || ev.label === eventFilter)
                  .sort((a, b) => b.ts - a.ts)
                  .map((ev) => (
                    <div
                      key={ev.id}
                      onClick={() => {
                        // fly to event location and sync video
                        ...
                        // mark as read
                        setEvents((prev) =>
                          prev.map((e) => (e.id === ev.id ? { ...e, seen: true } : e))
                        );
                      }}
                      style={{
          ...
          {missionActive && (
            <>
              <VideoReview
                ...
                style={
                  showFeed && videoExpanded // ✅ only when full width AND feed open
                    ? { paddingLeft: 140 }
                    : {}
                }
              />
